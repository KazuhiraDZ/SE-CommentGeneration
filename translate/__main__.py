...
#        config.score_functions = evaluation.name_mapping[config.score_function]
    ...
    # 用自定义的dict结构封装encoder和decoder，可以调用.name和.max_len属性
    ...
    with tf.Session(config=tf_config, graph=transfer_graph) as sess:
        ...
        # 拿到最好的保存模型地址
        best_checkpoint = os.path.join(config.checkpoint_dir, 'translate-376389')
        ...
        if args.save:
            ...
            eval_file_path = config.checkpoint_dir + "evaluation.pkl"
            with open(eval_file_path, 'wb') as f:
                pickle.dump(model.training.scores, f)
            print("eval file is stored in " + eval_file_path)
