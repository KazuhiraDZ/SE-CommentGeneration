...
class TranslationModel:
    ...
    def evaluate(self, score_function, on_dev=True, output=None, remove_unk=False, max_dev_size=None,
        ...
        if on_dev:
            ...
            filenames = [['../data_version2.0/test/test.token.code', '../data_version2.0/test/test.token.nl']]
    ...
    def train_step(self, steps_per_checkpoint, model_dir, steps_per_eval=None, max_steps=0,
        ...
        #if self.global_step.eval() == 278:
        #    print(next(self.batch_iterator))
        #try:
        ...
        # print information
        print('epoch ' + str(epoch) + ' │ global step: '+str(global_step)+' │ loss: ' + str(res.loss) + ' │ time: ' + str(self.training.time))

        # 当训练epoch达到某一值（训练的总数据量超过了某一限制）的时候
        ...
        if epoch >= 30 and steps_per_eval and global_step % steps_per_eval == 0 and 0 <= eval_burn_in <= global_step:
            ...
            print("current epoch: " + str(epoch) + " now is evaluating...")
            score, avg_score = self.evaluate(on_dev=False, **kwargs_)
            self.training.scores.append((epoch, global_step, score, avg_score))
