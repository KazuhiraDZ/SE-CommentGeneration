...
def multi_encoder(encoder_inputs, encoders, encoder_input_length, other_inputs=None, **kwargs):
    ...
    for encoder in encoders:
        ...
        # 定义embedding层的维度[单词量 x embedding维度]
    ...
    # 开始遍历encoder（其实这里就只有一个encoder）
    for i, encoder in enumerate(encoders):
        if encoder.use_lstm is False:
            encoder.cell_type = 'GRU'

        # 我们这里处理的是code，yaml中输入的encoder名字也是code，因此该层名字叫encoder_code
        with tf.variable_scope('encoder_{}'.format(encoder.name)):

            # 之前定义好的tensor
            # 分别是[b，v，h]的encoder_inputs_
            # 和[b,]的input_length
            ...
            if encoder.bidir:
                ...
                # encoder为1层的时候，直接调用cell拿到rnn的cell
                else:
            ...
            # 最终取得encoder_outputs, state以及input_length
...
def encoder_decoder(encoders, decoders, encoder_inputs, targets, feed_previous, align_encoder_id=0,
    ...
    # encoder部分的计算，这里似乎没有attention的内容
